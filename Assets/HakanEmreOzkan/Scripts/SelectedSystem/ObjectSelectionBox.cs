...
public class ObjectSelectionBox : MonoBehaviour
{
    Camera myCam;
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            {
                ObjectSelectionManager.instance.DeselectAll();
                SelectUnits();
            }
    ...
    void SelectUnits()
    {
        foreach (var unit in ObjectSelectionManager.instance.allObjectsList)
        {
            if (selectionBox.Contains(myCam.WorldToScreenPoint(unit.transform.position)))
            {
                if (!unit.gameObject.GetComponent<FriendlyCharacterController>().amIDeath)
                    ObjectSelectionManager.instance.DragSelect(unit);
            }
        }
    }
}
